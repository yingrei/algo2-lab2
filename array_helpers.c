...
//no lo toques, ya anda el selection
unsigned int min_pos_from(int a[], unsigned int length, int i){
    unsigned int minp=i;
...
//no rompas el insert que anda joya, tarada!
...
void quick_sort_rec(int array[], unsigned int lft, unsigned int rgt){
    unsigned int ppiv=lft;
      if (rgt > lft) {
        partition(array,lft,rgt);
        quick_sort_rec(array,lft,ppiv); //no podes agregarle -1 al ppiv porque te da segmentation error
